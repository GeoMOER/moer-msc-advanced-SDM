...
#'@description create virtual species based on bioclim variables
...
setwd("C:/Users/bald_local/DATA/Lehre/moer-msc-advanced-SDM/data/virtualSpecies")
...
![image](../assets/images/unit02/random_sp2.png)
...
The figure below demonstrates how these functions can transform an environmental variable.

![image](../assets/images/unit02/functions2.png)
*Image: : The values of an environmental variable transformed with a linear, beta, logistic and quadratic function.* 
...
params <- formatFunctions(bio_2 = c(fun = 'logisticFun', alpha = -0.1, beta = 0.25),
                          bio_5 = c(fun = 'quadraticFun', a = -1, b = 1, c = 20),
                          bio_11 = c(fun = 'betaFun', p1 = 1, p2 = 2, alpha = 0.2, gamma = 0.4))

# Generate the virtual species based on the defined parameters and formula
species1 <- generateSpFromFun(raster.stack = r[[c("bio_2", "bio_5", "bio_11")]],
                              parameters = params,
                              formula = "5.5 * bio_2^2  * sqrt(bio_5) + 0.5 * bio_11",  # Formula defining suitability
...
saveRDS(species1PA, "species1.RDS")
...
![image](../assets/images/unit02/function_species2.png)
